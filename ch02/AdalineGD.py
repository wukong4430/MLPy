# -*- coding: utf-8 -*-
# @Author: kicc
# @Date:   2018-01-07 12:26:12
# @Last Modified by:   kicc
# @Last Modified time: 2018-01-07 13:58:29
...
class AdalineGD(object):
    ...
    def fit(self, X, y):
        ...
        for i in range(self.n_iter):
            ...
            self.w_[0] += self.eta * errors.sum()  # zero-weight
            cost = (errors**2).sum() / 2.0
            # we collect the cost values in a list self.cost_ to check
            # if the algorithm converged after training.
...
def SSE_Epochs():
    """ Show cost against the number of epochs for two 
        different learning rate.(0.01 and 0.0001)
    ...
    ada3 = AdalineGD(n_iter=10, eta=0.0003).fit(X, y)
...
def test2():
    ...
    # After standardization, we will train the Adaline again and
    # see that it now converges using a eta=0.01:
